...
public class Searcher extends Parametre {
	...
	public static ArrayList<String> fuzzySearch(String s, int topDoc)
			throws IOException, ParseException {
		new Searcher("", s);
		ArrayList<String> result = new ArrayList<String>();
		...
		for (int i = 0; i < QueryTerms.length; i++) {
			FuzzyQuery query = new FuzzyQuery(new Term("contents", QueryTerms[i].trim()));
	...
	public static void main(String[] args) throws IOException, ParseException {
		...
		resultat = fuzzySearch(q, 10);
		Iterator<String> iterator = resultat.iterator();
		while (iterator.hasNext()) {
			System.out.println(iterator.next());
		}
		System.out.println("*******************search full text FR*********************************");
		resultat = Search(q, 10, "FR");
		iterator = resultat.iterator();
		while (iterator.hasNext()) {
			System.out.println(iterator.next());
		}
		System.out.println("*******************search full text EN*********************************");
